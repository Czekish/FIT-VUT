...
**/
    ...
    while(foo_node != NULL) {
        if (Content < foo_node->Cont) {
            parent = foo_node;
            foo_node = foo_node->LPtr;
        } else if (Content > foo_node->Cont) {
            parent = foo_node;
            foo_node = foo_node->RPtr;
        } else {
            return;
        }
    }

    tBTNodePtr node_new = malloc(sizeof(struct tBTNode));
    node_new->RPtr = NULL;
    node_new->LPtr = NULL;
    node_new->Cont = Content;

    if (parent->Cont < Content) {
        parent->RPtr = node_new;
    } else {
        parent->LPtr = node_new;
    }
    return;
...
**/
	...
	while(SEmptyP(&stack) == FALSE) {
	    // Nacteme prvek z vrcholu zasobniku
	    RootPtr = STopPopP(&stack);
	    // Zpracovavame uzel
	    BTWorkOut(RootPtr);
	    // Prochazime doprava
...
**/
    ...
    while(ptr != NULL) {
        ...
        // Posuneme se doleva
        ptr = ptr->LPtr;
        // Ulozime hodnotu bool true, na znameni, ze jsme navstivili uzel poprve
        SPushB(StackB, TRUE);
    }
    return;
}
...
**/
    ...
    // Zasobnik bool hodnot
    tStackB stackBool;
    SInitB(&stackBool);
    // Zasobnik pointeru na uly
    tStackP stackPtrs;
    SInitP(&stackPtrs);

    // Prochazime zleva
    Leftmost_Postorder(RootPtr, &stackPtrs, &stackBool);

    while(SEmptyP(&stackPtrs) == FALSE) {
        // Nacitame uzel z vrcholu zasobniku
        RootPtr = STopPopP(&stackPtrs);
        // Rozhodujeme se, kterym smerem pujdeme
        if(STopPopB(&stackBool)) {
            // Vratime uzel na zasobnik
            SPushP(&stackPtrs, RootPtr);
            // Rikame, ze jsme uzel jiz navstivili
            SPushB(&stackBool, FALSE);
            Leftmost_Postorder(RootPtr->RPtr, &stackPtrs, &stackBool);
        }
        // Jiz jsme uzel navstivili
        else {
            BTWorkOut(RootPtr);
        }
    }
		...
		while(SEmptyP(&stackPtrs) == FALSE) {
		    *RootPtr = STopPopP(&stackPtrs);
		    if((*RootPtr)->RPtr != NULL){
		        Leftmost_Inorder((*RootPtr)->RPtr, &stackPtrs);
		    }
		    free(*RootPtr);
		}
		*RootPtr = NULL;
